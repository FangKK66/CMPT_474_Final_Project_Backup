...
import static com.atguigu.eduorder.utils.ConstantPropertiesUtil.*;
...
public class PayLogServiceImpl extends ServiceImpl<PayLogMapper, PayLog> implements PayLogService {
    ...
    public Map createNative(String orderNo) {
        try {
            ...
            //设置xml格式的参数
            client.setXmlParam(WXPayUtil.generateSignedXml(m, PARTNER_KEY));
    ...
    public void updateOrdersStatus(Map<String, String> map) {
        ...
        wrapper.eq("order_no", orderNo);
        Order order = orderService.getOne(wrapper);
//        orderService.updateStatus(orderNo);   //由于需要取出订单金额，所以这里不能直接这样写，还需要再查询一次
        //更新订单表订单状态
        if (order.getStatus() == 1) {
            return;
        }
        order.setStatus(1);//1代表已经支付
        order.setPayType(1);//1 代表微信支付
        orderService.updateById(order);

        //向支付表添加支付记录    订单金额0.01和支付方式微信支付都默认
