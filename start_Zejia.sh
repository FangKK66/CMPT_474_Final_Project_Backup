...
cd $HOME/SFU-CMPT474-Group_Project-NewRepo/frontend-admin
gcloud builds submit --tag  gcr.io/$PROJECT_ID/frontend-admin
gcloud run deploy frontend-admin --image gcr.io/$PROJECT_ID/frontend-admin --platform=managed --region=us-central1 --port=9528 --no-allow-unauthenticated

cd $HOME/SFU-CMPT474-Group_Project-NewRepo/frontend-user
gcloud builds submit --tag  gcr.io/$PROJECT_ID/frontend-user
gcloud run deploy frontend-user --image gcr.io/$PROJECT_ID/frontend-user --platform=managed --region=us-central1 --port=9000 --no-allow-unauthenticated
